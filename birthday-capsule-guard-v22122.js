...
function openBirthdays(){
  try{
    if(typeof window.AgendaBirthdays?.open==='function'){
      window.AgendaBirthdays.open('week');
      return true;
    }
  }catch(e){console.warn('Cumpleaños: no se pudo abrir',e)}
  return false;
}
function bindCard(card){
  if(!card)return;
  card.setAttribute('aria-label','Abrir cumpleaños');
  card.setAttribute('role','button');
  card.tabIndex=0;
  card.onclick=(ev)=>{ev.preventDefault();ev.stopPropagation();openBirthdays()};
  card.onkeydown=(ev)=>{if(ev.key==='Enter'||ev.key===' '){ev.preventDefault();openBirthdays()}};
}
function repair(){
  installStyle();
...
function delegatedOpen(ev){
  const card=ev.target?.closest?.('#apBirthdayCard');
  if(!card)return;
  ev.preventDefault();
  ev.stopPropagation();
  openBirthdays();
...
window.AgendaBirthdayCapsuleGuard={version:VERSION,repair,open:openBirthdays};
})();

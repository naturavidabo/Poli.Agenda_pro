...
const VERSION_RE=/\bv?2\.(?:14\.1|16\.\d+|17\.\d+|18\.\d+|19\.0)\b/g;
...
function hideBadFloating(){document.getElementById('serviceGroupsQuick')?.remove();document.querySelectorAll('.sg-quick').forEach(x=>x.remove());if(!isHorario())return;document.querySelectorAll('button').forEach(b=>{const t=(b.textContent||'').trim(),cs=getComputedStyle(b),r=b.getBoundingClientRect(),floating=cs.position==='fixed'||cs.position==='absolute';if(floating&&(t==='+'||/agregar|nuevo/i.test((b.getAttribute('aria-label')||'')+' '+(b.title||'')))&&r.width>=45&&r.height>=45)b.style.setProperty('display','none','important')})}
function findScheduleAccess(main){
  const candidates=[...main.querySelectorAll('button,a,[role="button"],.card,.home-card,.dashboard-card,.quick-card,.action-card')];
  let hit=candidates.find(el=>/^mi\s+horario$/i.test((el.textContent||'').replace(/\s+/g,' ').trim()));
  if(!hit)hit=candidates.find(el=>/\bmi\s+horario\b/i.test((el.textContent||'').replace(/\s+/g,' ').trim()));
  if(!hit)hit=candidates.find(el=>/\bhorario\b/i.test((el.textContent||'').replace(/\s+/g,' ').trim())&&!el.closest('.bottom-nav'));
  return hit||null;
}
function installGroupAccess(){
  ...
  const schedule=findScheduleAccess(main);
  const desiredParent=schedule?.parentElement||main;
  if(old&&old.parentElement===desiredParent){
    if(schedule&&old.previousElementSibling!==schedule)schedule.insertAdjacentElement('afterend',old);
    return;
  }
  old?.remove();
  ...
  if(schedule){card.classList.add('sgha-near-schedule');schedule.insertAdjacentElement('afterend',card)}else main.prepend(card);
}
